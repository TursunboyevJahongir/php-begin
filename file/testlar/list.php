<?php
    ...
    // else {}
        ...
        if($f){
            ...
            echo $_POST['find'];
            var_dump($find);
            $query =$connection->query("SELECT * FROM `user` WHERE `first_name` LIKE '%'.$find.'%' 
    ...
    $query = $connection->query('SELECT COUNT(*) as elements FROM user');
    ...
    $query = $connection->query(
        'SELECT * FROM user ORDER BY id DESC LIMIT '.$page_limit.' OFFSET '.$offset
    );
    if($page<=3){
        $begin=1;
        $end=5;
